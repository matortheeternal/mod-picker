...
def create_quotes(game_name)
  filename = game_name.downcase.gsub(" ", "_") + ".json"
  file = File.read(Rails.root.join("db", "quotes", filename))
  game = Game.find_by(display_name: game_name)
  JSON.parse(file).each do |quote|
    quote["game_id"] = game.id
    Quote.create!(quote.with_indifferent_access)
  end
end
