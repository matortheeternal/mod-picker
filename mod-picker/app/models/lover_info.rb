...
class LoverInfo < ActiveRecord::Base
  ...
  def lovers_lab_url
    "https://www.loverslab.com/files/file/#{id}"
  end
  ...
  def scrape
    ...
    self.mod_name = doc.at_css(".ipsType_pagetitle").children[1].text.strip
    ...
    date_submitted_str = file_information_items[0].children[1].text.strip
    self.date_submitted = DateTime.parse(date_submitted_str, lab_date_format)
    date_updated_str = file_information_items[1].children[1].text.strip
    self.date_updated = DateTime.parse(date_updated_str, lab_date_format)

    # scrape statistics
    if Rails.application.config.scrape_lab_statistics
      self.file_size = file_information_items[2].children[1].text.gsub('MB', '').to_f * 1024 * 1024
      self.views = file_information_items[3].children[1].text.to_i
      self.downloads = file_information_items[3].children[1].text.gsub(',', '').to_i
