class CompatibilityNotesController < ContributionsController
  ...
  def index
    ...
    @compatibility_notes = CompatibilityNote.preload(:editor, :editors, :submitter => :reputation).includes(:first_mod, :second_mod).accessible_by(current_ability).filter(filtering_params).sort(params[:sort]).paginate(:page => params[:page])
    count = CompatibilityNote.preload(:submitter => :reputation).accessible_by(current_ability).filter(filtering_params).count
