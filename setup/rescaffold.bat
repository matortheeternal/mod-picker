...
call rails g scaffold mods mod_id:integer game_id:integer name:text aliases:text is_utility:boolean has_adult_content:boolean primary_category_id:integer secondary_category_id:integer --force --skip-migration
...
call rails g scaffold plugins pl_id:integer mv_id:integer filename:text author:text description:text hash:string --force --skip-migration
:: plugins that serve as masters for other plugins
call rails g scaffold masters mst_id:integer pl_id:integer --force --skip-migration
...
call rails g scaffold mod_lists ml_id:integer game:text created_by:integer is_collection:boolean is_public:boolean has_adult_content:boolean status:integer created:datetime completed:datetime description:text --force --skip-migration
...
:: call rails g scaffold users user_id:integer username:string email:string user_level:integer title:string avatar:text joined:datetime active_ml_id:integer active_mc_id:integer
...
call rails g scaffold comments c_id:integer parent_comment:integer submitted_by:integer hidden:boolean submitted:datetime edited:datetime text_body:text --force --skip-migration

:: mod reviews submitted by users
call rails g scaffold reviews r_id:integer submitted_by:integer mod_id:integer hidden:boolean rating1:TINYINT rating2:TINYINT rating3:TINYINT rating4:TINYINT:rating5 TINYINT submitted:datetime edited:datetime text_body:text --force --skip-migration

:: installation notes submitted by users
call rails g scaffold installation_notes in_id:integer submitted_by:integer mv_id:integer always:boolean note_type:integer submitted:datetime edited:datetime text_body:text --force --skip-migration

:: compatibility notes submitted by users
call rails g scaffold compatibility_notes cn_id:integer submitted_by:integer mod_mode:integer compatibility_patch:integer compatibility_status:integer --force --skip-migration
...
call rails g scaffold incorrect_notes inc_id:integer r_id:integer cn_id:integer in_id:integer submitted_by:integer reason:text --force --skip-migration
